...
# ---- piper stage ----
# TTS voice model (en_US-joe-medium, ~60 MB, male) + its config, baked in so
...
RUN mkdir -p /opt/piper/voices \
    && curl -fsSL -o /opt/piper/voices/en_US-joe-medium.onnx \
        https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/joe/medium/en_US-joe-medium.onnx \
    && curl -fsSL -o /opt/piper/voices/en_US-joe-medium.onnx.json \
...
# Bake the faster-whisper distil-small.en model (downloaded from HF at build
# time): English-only, ~317 MB, accuracy on par with whisper-small but faster
# on the CPU — better than tiny for recitations and long-form speech.
RUN python3 -c "from huggingface_hub import snapshot_download; snapshot_download('Systran/faster-distil-whisper-small.en', local_dir='/opt/whisper/model')"
...
RUN mkdir -p /opt/affirmations \
    && echo "Sir."                 | piper -m /opt/piper/voices/en_US-joe-medium.onnx -f /opt/affirmations/00.wav \
    && echo "Listening, Sir."      | piper -m /opt/piper/voices/en_US-joe-medium.onnx -f /opt/affirmations/01.wav \
    && echo "Right here, Sir."     | piper -m /opt/piper/voices/en_US-joe-medium.onnx -f /opt/affirmations/02.wav \
    && echo "Yes, Sir?"            | piper -m /opt/piper/voices/en_US-joe-medium.onnx -f /opt/affirmations/03.wav \
    && echo "At your service, Sir." | piper -m /opt/piper/voices/en_US-joe-medium.onnx -f /opt/affirmations/04.wav \
    && echo "I'm here, Sir."       | piper -m /opt/piper/voices/en_US-joe-medium.onnx -f /opt/affirmations/05.wav \
    && echo "How can I help, Sir?" | piper -m /opt/piper/voices/en_US-joe-medium.onnx -f /opt/affirmations/06.wav \
    && echo "Ready when you are, Sir." | piper -m /opt/piper/voices/en_US-joe-medium.onnx -f /opt/affirmations/07.wav \
    && echo "Standing by, Sir."    | piper -m /opt/piper/voices/en_US-joe-medium.onnx -f /opt/affirmations/08.wav \
    && echo "Go ahead, Sir."       | piper -m /opt/piper/voices/en_US-joe-medium.onnx -f /opt/affirmations/09.wav \
    && echo "Processing, Sir."     | piper -m /opt/piper/voices/en_US-joe-medium.onnx -f /opt/affirmations/processing.wav
...
ENV PIPER_VOICE=/opt/piper/voices/en_US-joe-medium.onnx
